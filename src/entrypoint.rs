...
fn process_instruction(
    ...
    match InstructionSet::try_from(ix_disc)? {
        InstructionSet::InitializeState => {
            ...
            process_initialize_state(accounts, instruction_data)
        }
        InstructionSet::AddMapping => {
            msg!("Adding mapping");
            process_add_mapping(accounts, instruction_data)
        }
        InstructionSet::CloseMapping => {
            msg!("Closing mapping");
            process_close_mapping(accounts, instruction_data)
        }
    }
}
